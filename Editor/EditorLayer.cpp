...
namespace Engine3D{
	...
	// 	playIcon = Texture2D::Create("assets2/icons/PlayButton.png");
	// 	stopIcon = Texture2D::Create("assets2/icons/StopButton.png");

	// 	FrameBufferSpecifications specifications;
	// 	specifications.attachments = { FrameBufferTextureFormat::RGBA8, FrameBufferTextureFormat::RED_INTEGER, FrameBufferTextureFormat::DEPTH24STENCIL8 };
	// 	specifications.width = 1280;
	// 	specifications.height = 720;

	// 	framebuffers = FrameBuffer::Create(specifications);
	...
	// 	sceneHeirarchyPanel.setContext(activeScene);
	// }

    // void EditorLayer::onDetach() {}

    // void EditorLayer::onUpdate(Timestep ts) {
	// 	if(FrameBufferSpecifications spec = framebuffers->getSpecifications();
	// 			viewportSize.x > 0.0f and viewportSize.y > 0.0f &&
	// 			(spec.width != viewportSize.x || spec.height != viewportSize.y)){
	// 		framebuffers->resize((uint32_t)viewportSize.x, (uint32_t)viewportSize.y);
	// 		editorCamera.setViewportSize(viewportSize.x, viewportSize.y);
	// 		activeScene->onViewportResize(viewportSize.x, viewportSize.y); // viewport resizing every time the window size is changed
	// 	}
	...
	// 	SceneState state = (SceneState)sceneState;
	...
	// 		int pixel = framebuffers->readPixel(1, currentMouseX, currentMouseY);
	// 		coreLogInfo("Pixel Data is {}", pixel);
	// 		if(pixel == -1 || pixel > 1000000){
	// 			hoveredEntity = Entity();
	// 		}
	// 		else{
	// 			hoveredEntity = Entity((entt::entity)pixel, activeScene.get());
	// 		}

	// 		/* hoveredEntity = pixel == -1 ? Entity() : Entity((entt::entity)pixel, activeScene.get()); */
	// 	}
	...
	// 	Application::Get().getImGuiLayer()->setBlockEvents(!isViewportFocused && !isViewportHovered); // if either out of focused or hovered
	...
	// 		framebuffers->resize((uint32_t)viewportPanelSize.x, (uint32_t)viewportPanelSize.y);
	// 		viewportSize = {viewportPanelSize.x, viewportPanelSize.y};
	// 	}

	// 	// By passing this renderer ID, this gives us the ID of the texture that we want to render.
	// 	uint32_t textureID = framebuffers->getColorAttachmentRendererID(); // Getting color buffer from frame buffer
	...
	// 	Entity selectedEntity = sceneHeirarchyPanel.getCurrentSelectedEntity();
	...
	// 		ImGuizmo::Manipulate(glm::value_ptr(cameraView), glm::value_ptr(cameraProjection), (ImGuizmo::OPERATION)gizmoType, ImGuizmo::LOCAL, glm::value_ptr(transform), nullptr, isSnap ? snapValues : nullptr);
	...
	// 		if(isViewportHovered && !ImGuizmo::IsUsing() && !InputPoll::isKeyPressed(Key::LeftAlt))
	...
	// 	SceneSerializer serializer(activeScene);
	// 	serializer.deserialize("assets2/scene/3DGreenCubeWorks.engine");
	// }
	...
	void EditorLayer::onAttach(){
		...
		_checkerboardTexture = Texture2D::Create("assets2/Checkerboard.png");
		_iconPlay = Texture2D::Create("assets2/icons/PlayButton.png");
		_iconStop = Texture2D::Create("assets2/icons/StopButton.png");
	...
	void EditorLayer::ui_toolBar(){
		...
		//! @note Handling button clicking on ImGui!
		if(ImGui::ImageButton((ImTextureID)icon->getRendererID(), ImVec2{size, size}, ImVec2(0, 0), ImVec2(1, 1))){
