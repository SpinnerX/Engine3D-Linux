...
namespace Engine3D{
	...
	const std::filesystem::path assetPath = "assets2";
	...
	// void SceneHeirarchyPanel::onUIRender(){
	// 	// @note Drawing this whole scene heirarchy.
	// 	// @note Setting up ImGui
	// 	ImGui::Begin("Scene Heirarchy");
	// 	sceneContext->_registry.each([&](auto entityID){
	...
	// 	ImGuiTreeNodeFlags flags = ((currentSelectedEntity == entity) ? ImGuiTreeNodeFlags_Selected : 0) | ImGuiTreeNodeFlags_OpenOnArrow;
	...
	// 		if(!currentSelectedEntity.hasComponent<CameraComponent>()){
	...
	// 					const std::filesystem::path texturePath = std::filesystem::path(assetPath) / filepath;
	// 					/* component.texture = Texture2D::Create(texturePath); */
	// 					component.texture = Texture2D::Create(texturePath.generic_string());
	// 				}
	...
	void SceneHeirarchyPanel::drawComponents(Entity entity){
		...
		drawComponent<SpriteRendererComponent>("Sprite Renderer", entity, [](auto& component){
				...
				if(ImGui::BeginDragDropTarget()){
					if(const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("CONTENT_BROWSER_ITEM")){
						const char* filepath = (const char*)payload->Data;
						std::filesystem::path texturePath = std::filesystem::path(assetPath) / filepath;
						std::string t = texturePath.string();
						component.texture = Texture2D::Create(t);
					}
