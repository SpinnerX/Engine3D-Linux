...
namespace Engine3DLinux{
	...
	static GLenum engineFBTexturFormatToGL(FrameBufferTextureFormat format){
		switch (format) {
			case Engine3DLinux::FrameBufferTextureFormat::RGBA8: return GL_RGBA8;
			case Engine3DLinux::FrameBufferTextureFormat::RED_INTEGER: return GL_RED_INTEGER;
			default: return 0;
		}
		return 0;
	}
		...
		if(colorAttachments.size() > 0){
			for(size_t i = 0; i < colorAttachments.size(); i++){
				...
				switch(colorAttachments[i].textureFormat){
					case Engine3DLinux::FrameBufferTextureFormat::RGBA8:
					{
						...
						break;
					case Engine3DLinux::FrameBufferTextureFormat::RED_INTEGER:
						{
							if(multisample){
								glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, _specifications.samples, GL_RGBA8, _specifications.width, _specifications.height, GL_FALSE);
							}
	...
	void OpenGLFrameBuffer::clearColorAttachment(uint32_t attachmentIndex, int data){
		...
		auto& spec = colorAttachments[attachmentIndex];
		glClearTexImage(colorAttachments[attachmentIndex].attachmentID, 0, engineFBTexturFormatToGL(spec.textureFormat), GL_INT, &data);
	}
