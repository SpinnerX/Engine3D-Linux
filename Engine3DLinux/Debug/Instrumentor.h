...
#define RENDER_PROFILE 1
	...
	#define RENDER_PROFILE_BEGIN_SESSION(name, filepath) ::Engine3DLinux::Instrumentor::get().beginSession(name, filepath);
	#define RENDER_PROFILE_END_SESSION() ::Engine3DLinux::Instrumentor::get().endSession();
	
	#define RENDER_PROFILE_SCOPE(name) ::Engine3DLinux::InstrumentorTimer timer##__LINE__(name);
